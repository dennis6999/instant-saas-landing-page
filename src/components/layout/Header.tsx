...
export function Header() {
  ...
  useEffect(() => {
    if (mobileOpen) {
      ...
      document.documentElement.classList.add("force-blur-repaint");
      // Remove after a tick to trigger repaint
      setTimeout(() => {
        document.body.classList.remove("force-blur-repaint");
        document.documentElement.classList.remove("force-blur-repaint");
      }, 50);
    }
  }, [mobileOpen]);

  useEffect(() => {
    if (mobileOpen && menuOverlayRef.current) {
      // Force a reflow on the overlay div to ensure backdrop-blur is applied
      void menuOverlayRef.current.offsetHeight;
    }
  }, [mobileOpen]);
  return (
    <>
      {mobileOpen && (
        <div ref={menuOverlayRef} className="fixed inset-0 z-[60] flex items-start justify-end bg-black/40 backdrop-blur-sm" onClick={() => setMobileOpen(false)}>
          <nav
            className="mt-4 mr-4 w-64 rounded-2xl bg-white/70 dark:bg-background/80 backdrop-blur-xl border border-white/30 dark:border-white/10 shadow-2xl p-7 flex flex-col gap-3 animate-fade-in-up glass-nav-menu z-[70]"
